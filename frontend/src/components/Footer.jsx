import { motion } from 'framer-motion'
import { Link } from 'react-router-dom'

export default function Footer() {
  const currentYear = new Date().getFullYear()

  const socialLinks = [
    { name: 'GitHub', url: 'https://github.com/cashkid12', icon: '💻', color: 'hover:text-electric' },
    { name: 'LinkedIn', url: 'https://linkedin.com/in/Joe Nthiga', icon: '💼', color: 'hover:text-blue-400' },
    { name: 'Twitter', url: 'https://twitter.com/VoiceOf254', icon: '🐦', color: 'hover:text-blue-300' },
    { name: 'Email', url: 'mailto:joenthiga678@gmail.com', icon: '✉️', color: 'hover:text-green-400' }
  ]

  const quickLinks = [
    { path: '/', label: 'Home' },
    { path: '/about', label: 'About' },
    { path: '/projects', label: 'Projects' },
    { path: '/skills', label: 'Skills' },
    { path: '/contact', label: 'Contact' }
  ]

  return (
    <footer className="bg-black border-t border-gray-900 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0">
        <motion.div 
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.03, 0.08, 0.03]
          }}
          transition={{
            duration: 10,
            repeat: Infinity,
            ease: "easeInOut"
          }}
          className="absolute bottom-0 left-0 w-72 h-72 bg-electric/10 rounded-full blur-3xl"
        />
        <motion.div 
          animate={{
            scale: [1.2, 1, 1.2],
            opacity: [0.08, 0.03, 0.08]
          }}
          transition={{
            duration: 8,
            repeat: Infinity,
            ease: "easeInOut"
          }}
          className="absolute bottom-0 right-0 w-72 h-72 bg-blue-500/10 rounded-full blur-3xl"
        />
      </div>

      <div className="container mx-auto px-4 py-12 relative z-10">
        <div className="grid md:grid-cols-3 gap-8 mb-8">
          {/* Brand Section */}
          <div className="space-y-4">
            <Link to="/" className="flex items-center gap-3 group">
              <motion.div 
                whileHover={{ scale: 1.15, rotate: 10 }}
                className="w-12 h-12 bg-gradient-to-br from-electric to-blue-600 rounded-lg flex items-center justify-center shadow-lg shadow-electric/30"
              >
                <span className="text-white font-bold text-xl">JN</span>
              </motion.div>
              <div>
                <span className="text-2xl font-bold bg-gradient-to-r from-electric to-blue-400 bg-clip-text text-transparent">
                  Joe Nthiga
                </span>
                <p className="text-gray-400 text-sm">Full-Stack Developer</p>
              </div>
            </Link>
            <p className="text-gray-400 leading-relaxed max-w-md">
              Building secure, modern web applications with cutting-edge technologies. 
              Passionate about creating solutions that make a difference while ensuring digital safety.
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <motion.span
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ duration: 2, repeat: Infinity }}
              >
                ⚡
              </motion.span>
              Quick Links
            </h3>
            <div className="space-y-3">
              {quickLinks.map((link) => (
                <motion.div key={link.path} whileHover={{ x: 5 }}>
                  <Link
                    to={link.path}
                    className="text-gray-400 hover:text-electric transition-all duration-300 flex items-center gap-2 group"
                  >
                    <motion.div 
                      className="w-1 h-1 bg-electric rounded-full"
                      initial={{ opacity: 0, scale: 0 }}
                      whileHover={{ opacity: 1, scale: 1 }}
                      transition={{ duration: 0.2 }}
                    />
                    {link.label}
                  </Link>
                </motion.div>
              ))}
            </div>
          </div>

          {/* Connect */}
          <div>
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <motion.span
                animate={{ rotate: [0, 20, 0] }}
                transition={{ duration: 2, repeat: Infinity }}
              >
                🌐
              </motion.span>
              Let's Connect
            </h3>
            <div className="space-y-3">
              {socialLinks.map((social) => (
                <motion.a
                  key={social.name}
                  href={social.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center gap-3 text-gray-400 ${social.color} transition-all duration-300 group`}
                  whileHover={{ x: 5, scale: 1.02 }}
                >
                  <span className="text-lg transform group-hover:scale-110 transition-transform duration-300">
                    {social.icon}
                  </span>
                  <span>{social.name}</span>
                </motion.a>
              ))}
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="pt-8 border-t border-gray-900 flex flex-col md:flex-row justify-between items-center gap-4">
          <motion.p 
            className="text-gray-500 text-sm"
            animate={{ opacity: [0.7, 1, 0.7] }}
            transition={{ duration: 3, repeat: Infinity }}
          >
            © {currentYear} Joe Nthiga. All rights reserved.
          </motion.p>
          <div className="flex items-center gap-6 text-sm text-gray-500">
            <motion.span
              whileHover={{ scale: 1.05, color: "#1E90FF" }}
              className="transition-colors duration-300 cursor-default"
            >
              Built with React & Tailwind CSS
            </motion.span>
            <motion.div 
              className="w-1 h-1 bg-electric rounded-full"
              animate={{ scale: [1, 1.5, 1] }}
              transition={{ duration: 2, repeat: Infinity }}
            />
            <motion.span
              whileHover={{ scale: 1.05 }}
              className="transition-colors duration-300 cursor-default"
            >
              Made with 💙
            </motion.span>
          </div>
        </div>
      </div>
    </footer>
  )
}